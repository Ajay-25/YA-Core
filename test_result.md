...
- task: "Health check API"
  ...
  agent: "main"
  comment: "GET /api/health returns {status: ok, app: 'YA Core VRP'} - verified via curl" - working: true
  agent: "testing"
...
- task: "Profile ensure API"
  ...
  agent: "main"
  comment: "POST /api/profile/ensure - creates profile via service role. Returns 401 without auth token." - working: true
  agent: "testing"
...
- task: "Admin set role API"
  ...
  status_history: - working: "NA"
  agent: "main"
  comment: "POST /api/admin/set-role - requires admin auth, changes user role" - working: true
  agent: "testing"
  comment: "✅ VERIFIED: Auth guard working correctly - returns 401 'Unauthorized' without auth token"
