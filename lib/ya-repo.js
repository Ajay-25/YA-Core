...
export async function listStockItems() {
  return allRows(await db.execute(sql`SELECT * FROM stock_items ORDER BY category, name`))
}
...
const MAX_ISSUE_BATCH_LINES = 25

/**
 * Issue several catalog lines in one request. Validates total qty per item_id against
 * current_quantity, then applies lines in order (separate log rows per line).
 * @param {{ volunteerProfileId: string, issuedByProfileId: string, payment_method: string, lines: { itemId: string, qty: number }[] }} params
 */
export async function issueInventoryLines({
  volunteerProfileId,
  issuedByProfileId,
  payment_method,
  lines,
  ...
  if (!lines || !Array.isArray(lines) || lines.length === 0) {
    return { error: 'At least one line item required', status: 400 }
  }
  if (lines.length > MAX_ISSUE_BATCH_LINES) {
    return {
      error: `At most ${MAX_ISSUE_BATCH_LINES} lines per batch`,
      status: 400,
    }
  }
  ...
  for (const line of lines) {
    ...
    if (!itemId || !UUID_RE.test(String(itemId)) || Number.isNaN(q) || q <= 0) {
      return { error: 'Each line needs a valid item_id and quantity_issued > 0', status: 400 }
    }
    needByItem.set(itemId, (needByItem.get(itemId) || 0) + q)
  }

  const running = new Map()
  for (const itemId of needByItem.keys()) {
    const item = await getInventoryItem(itemId)
    if (!item) {
      return { error: 'One or more items were not found', status: 404 }
    }
    ...
    if (have < need) {
      ...
      return {
        error: `Insufficient stock for ${label || 'item'}. Available: ${have}, requested: ${need}`,
        status: 400,
      }
    }
    running.set(itemId, { item, remaining: have })
  }

  const logs = []
  for (const line of lines) {
    const q = Number(line.qty)
    const meta = running.get(line.itemId)
    const unitPrice = Number(meta.item.unit_price) || 0
    const amount_due = Math.round(q * unitPrice * 100) / 100
    const log = await insertInventoryIssue({
      volunteerProfileId,
      itemId: line.itemId,
      issuedByProfileId,
      qty: q,
    ...
    const newRemaining = meta.remaining - q
    await decrementInventoryItemQty(line.itemId, newRemaining)
    meta.remaining = newRemaining
    logs.push(log)
  }
    ...
    volunteerIds.length > 0
      ? allRows(
          await db.execute(sql`
            SELECT id, user_id, full_name, ya_id FROM profiles_core
            WHERE id IN (${sql.join(
              volunteerIds.map((id) => sql`${id}::uuid`),
              sql`, `
            )})
          `)
        )
...
export async function listStockAuditForItem(itemId) {
  ...
  const profiles = allRows(
    await db.execute(sql`
      ...
      WHERE id IN (${sql.join(
        adjustedByIds.map((id) => sql`${id}::uuid`),
        sql`, `
      )})
    `)
  )
